...
/// ailia Document Loader のネイティブライブラリのファイル名
String ailiaDocumentLoaderLibraryName() {
  if (Platform.isAndroid || Platform.isLinux) {
    return 'libailia_document_loader.so';
  }
  if (Platform.isMacOS) {
    return 'libailia_document_loader.dylib';
  }
  if (Platform.isWindows) {
    return 'ailia_document_loader.dll';
  }
  return 'internal';
}

/// ailia Document Loader のネイティブライブラリを開く
///
...
DynamicLibrary openAiliaDocumentLoaderLibrary() {
  if (Platform.isIOS) {
    return DynamicLibrary.process();
  }
  final String name = ailiaDocumentLoaderLibraryName();
  try {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (file.existsSync()) {
      ...
      for (final dynamic package in config['packages'] as List<dynamic>) {
        if (package['name'] == 'ailia_document_loader') {
          final Uri rootUri =
